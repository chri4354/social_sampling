extensions [stats]
turtles-own [a b p eps H T a_prev b_prev]
;;;;;;;;;;;;;;;;;;;;;;;;
;;; Setup Procedures ;;;
;;;;;;;;;;;;;;;;;;;;;;;;
...
to go
  if truth [
    ask n-of nscouts turtles [world-sample]
    set truth false
  ]
  if (ticks mod 10000000) = 0 [; use ratio instead
    ifelse correlated [
      ask n-of nscouts turtles [world-sample]
    ]
    [
      ask n-of 10 turtles [
        world-sample
        ask turtle-set [other-end] of my-links [world-sample]
      ]
    ]
  ]
  ask turtles [social-sample]
...
to world-sample
  let s bool2num (random-float 1 < .5) ; sample the random variable
  set H (H + s)
  set T (T + (1 - s))
  set a_prev a
  set b_prev b
  set a (a + s) ; update internal model based on sample
  set b (b + (1 - s))
  set p (a / (a + b)); MAP
  set color scale-color red p 0 1
end

to social-sample
  let c n-social-samples
  loop [
    let advisor [other-end] of one-of my-links
    ask advisor[
      let x random-gamma a 1
      set eps ( x / ( x + random-gamma b 1) ) ;; set eps a random number from distribution Beta(a,b) (between 0 and 1)
    ]
    let advice bool2num (random-float 1 < [eps] of advisor); advice
    let rel advice-reliability a b [eps] of advisor; probability of the advice given my model
    let next_a (a + advice * rel); update internal model
    ;set rel advice-reliability a b (1 - advice); probability of the advice given my model
    let next_b (b + (1 - advice) * rel); update internal model
    set a next_a
    set b next_b
    set p (a / (a + b)); MAP
    set color scale-color red p 0 1
...
to social-sample-1 ; compare my model and advisor's model's likelihoods
  let advisor [other-end] of one-of my-links
  let a_adv [a] of advisor ; a parameter of the advisor's model
  let b_adv [b] of advisor ; b parameter of the advisor's model
  let data  (H / (H + T))
  let likelihood_self ((data ^ (a - 1) * (1 - data) ^ (b - 1))  / stats:bigBeta a b)
  let likelihood_adv ((data ^ (a_adv - 1) * (1 - data) ^ (b_adv - 1))  / stats:bigBeta a_adv b_adv)

  let x random-gamma a_adv 1
  let sample ( x / ( x + random-gamma b_adv 1) ) ;; sample a random number from distribution Beta(a,b) (between 0 and 1) from the advisor's model
  let advice bool2num (random-float 1 < sample); advice
  let map_adv (a_adv / (a_adv + b_adv))

  let prior_self (a_prev / (a_prev + b_prev))
  let prior_adv advice-reliability a_prev b_prev advice

  let p_accept ((likelihood_adv * prior_adv) / (likelihood_self * prior_self))
  if (random-float 1 < p_accept) [
    set a_prev a
    set b_prev b
    set a a_adv ; update
    set b b_adv ; update
...
to-report advice-reliability [alpha beta adv]
  ;let counter 0
  ;foreach n-values 1000 [1] [
  ;  let x random-gamma alpha 1
  ;  let s ( x / ( x + random-gamma beta 1) ) ;; draw a random number from distribution Beta(alpha,beta) (between 0 and 1)
  ;  if abs(s - adv) < .1 [set counter counter + 1]; adv is the advice received
  ;]
  ;report (counter / 1000)
  let denominator stats:bigBeta alpha beta
  if denominator = 0 [set denominator 0.00001]
  report (adv ^ (alpha - 1) * (1 - adv) ^ (beta - 1))  / denominator
end


to plot_beta [alpha beta]
  let mylist []
  foreach n-values 1000 [1] [
    let x random-gamma alpha 1
    let s ( x / ( x + random-gamma beta 1) ) ;; draw a random number from distribution Beta(a,b) (between 0 and 1)
    set mylist lput s mylist
  ]
...
"" ""
PENS
"default" 0.1 1 -16777216 true "" "histogram [p] of turtles"
...
687
661
Average neighbors' opinion distance
NIL
NIL
0.0
1.0
...
"" ""
PENS
"default" 0.1 1 -16777216 true "" "let mylist []\nask turtles [\n    let my-neighbors turtle-set [other-end] of my-links\n    let x abs(p - mean [p] of my-neighbors)\n    set mylist lput x mylist\n]\nset-current-plot \"Average neighbors' opinion distance\"\nhistogram mylist"
